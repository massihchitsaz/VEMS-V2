...
function pick(v: any, fields: string[]) {
  ...
  for (const key of fields) {
    if (Object.prototype.hasOwnProperty.call(v, key)) out[key] = normalize(v[key]);
  }
  return out;
}
...
export async function saveWarehouse(v: any) {
  const s = sb();
  const payload = { ...pick(v, warehouseFields), updated_at: new Date().toISOString() };
...
export async function saveLocation(v: any) {
  const s = sb();
  const payload = pick(v, locationFields);
  for (const key of ["capacity_qty", "temperature_min_c", "temperature_max_c"]) {
...
export async function saveLot(v: any) {
  ...
  if (!v.id) {
    const payload = {
      ...
      volume_cbm: v.volume_cbm ? Number(v.volume_cbm) : null,
    };
    const { data, error } = await s.rpc("inventory_receive_stock_v3", { p_payload: payload });
    if (error) throw error;
    return data;
  }
...
export async function addMovement(v: any) {
  const s = sb();
  const payload = pick(v, movementFields);
  payload.quantity = Number(v.quantity || 0);
  const { data, error } = await s.rpc("inventory_add_movement_v2", { p_payload: payload });
  if (error) throw error;
  return data;
}
...
export async function addReservation(v: any) {
  const s = sb();
  const payload = pick(v, reservationFields);
  payload.quantity = Number(v.quantity || 0);
  const { data, error } = await s.rpc("inventory_add_reservation_v2", { p_payload: payload });
  if (error) throw error;
  return data;
}
